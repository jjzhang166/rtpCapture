...
{   
    ...
    // add one session
    int fd0 = sdk.addCaptureRtpSession("movie/danbin.flv",45254);
    
    // add one session
    int fd1 = sdk.addCaptureRtpSession("movie/fangwei.flv",45255);
    
    // add one session
    int fd2 = sdk.addCaptureRtpSession("movie/yilian.flv",45256, 60000);
    
    // add one session
    int fd3 = sdk.addCaptureRtpSession("movie/rtsp.flv",45257);
    
    // add one session
    int fd4 = sdk.addCaptureRtpSession("movie/ps.vob",45258);
    ...
    std::cout << "RTP Session 4 : listen UDP port [45258] , capture file [ps.vob] , video frequency [90000]" << std::endl;
    /*       
